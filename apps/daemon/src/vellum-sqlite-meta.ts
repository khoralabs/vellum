...
export function getRosterActor(db: Database, principalUri: string): string | undefined {
  const row = db
...
export function upsertPreKeySecrets(
  db: Database,
  spkId: number,
  spkPrivHex: string,
...
export function loadPreKeySecrets(
  db: Database,
  otkId: number | null,
): { spkPriv: Uint8Array; otkPriv?: Uint8Array } {
  const spk = db
    .query<{ spk_priv_hex: string }, []>(`SELECT spk_priv_hex FROM vellum_prekey_secrets LIMIT 1`)
    .get();
...
export function upsertSessionKey(
  db: Database,
  sessionId: string,
